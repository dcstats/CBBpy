...
- `-v/--verbose` — log per-attempt retry diagnostics to the log file: every command
...
- `throttle` (default `1.0`): the mean delay, in seconds, each worker waits before scraping a game (jittered ±50% so workers don't fire in lockstep). Set `throttle=0` to disable the delay entirely.
- `n_jobs` (default `8`): the number of parallel workers. The work is I/O-bound, so this is a request-rate knob rather than a CPU one — the request rate is roughly `n_jobs / throttle` per second.
